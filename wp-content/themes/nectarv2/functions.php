...
//wp_enqueue_script('jquery-ui-core', 'https://ajax.googleapis.com/ajax/libs/jqueryui/1.10.4/jquery-ui.min.js', array('jquery'), '1.10.4');
//wp_enqueue_style('jquery-ui-core', 'https://ajax.googleapis.com/ajax/libs/jqueryui/1.10.4/themes/base/jquery-ui.css', false, '1.10.4', false);
...
add_filter("manage_edit-gallery_columns", "story_edit_columns");
add_filter("manage_edit-press_columns", "story_edit_columns");
...
function press_register() {
	...
	$labels = array(
		'name' => _x('The Press', 'post type general name'),
		'singular_name' => _x('Press Set', 'post type singular name'),
		'add_new' => _x('Add New', 'press item'),
		'add_new_item' => __('Add New Press Set'),
		'edit_item' => __('Edit Press Set'),
		'new_item' => __('New Press Set'),
		'view_item' => __('View Press Set'),
...
$press_box = array(
    ...
    'fields' => array(
        array(
            'name' => 'Caption',
            'desc' => 'The caption for this press item',
            'id' => $prefx . 'caption',
            'type' => 'text',
        )
...
add_action('admin_menu', 'press_add_box');

// Add meta box
function press_add_box() {
    global $press_box;

    add_meta_box($press_box['id'], $press_box['title'], 'press_show_box', $press_box['page'], $press_box['context'], $press_box['priority']);
}

// Callback function to show fields in meta box
function press_show_box() {
    global $press_box, $post;

    // Use nonce for verification
    echo '<input type="hidden" name="press_meta_box_nonce" value="', wp_create_nonce(basename(__FILE__)), '" />';
...
function press_save_data($post_id) {
    ...
    // verify nonce
    if (!wp_verify_nonce($_POST['press_meta_box_nonce'], basename(__FILE__))) {
        return $post_id;
    }
...
//add_action("manage_posts_custom_column",  "press_custom_columns");
//add_filter("manage_edit-press_columns", "press_edit_columns");
 
function press_edit_columns($press_columns){
  $press_columns = array(
    "cb" => "<input type=\"checkbox\" />",
    "title" => "Press Title",
    "description" => "Description",
  ...
  return $press_columns;
}
function press_custom_columns($press_columns){
	global $post;
 
	switch ($press_columns) {
		case "description":
			the_excerpt();
			break;
		case "thumb":
...
add_shortcode('gallery', 'press_gallery_shortcode');
 ...
 */
function press_gallery_shortcode($attr) {
	$post = get_post();
	...
	if ( is_feed() ) {
		...
		return $output;
	} else if ( 'press' == get_post_type() ) {
		$output = "\n";
		foreach ( $attachments as $att_id => $attachment ) {
			$img = wp_get_attachment_image($att_id, 'press-slide', true);
			$output .= '<div class="press_slide">';
			$output .= $img;
			$output .= '</div>';
		};
		return $output;
...
add_image_size( 'press-square', 300, 300, true );
add_image_size( 'press-slide', 960, 600, false );
